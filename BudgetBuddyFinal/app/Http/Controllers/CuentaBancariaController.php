...
class CuentaBancariaController extends Controller
{
    ...
    public function balanceCuenta()
    {
        $userId = Auth::id();
        $cuenta_bancaria = CuentaBancaria::where('usuario_id', $userId)->first();
        $sumaTotalGastos = Gasto::where('usuario_id', $userId)->sum('monto');
        $sumaTotalIngresos = Ingreso::where('usuario_id', $userId)->sum('monto');
        $monto = $cuenta_bancaria->monto - $sumaTotalIngresos - $sumaTotalGastos;
        $monto = number_format($monto, 2, '.', '');
        return response()->json(['balance' => $monto]);
    }

    public function GastoIngresoTotal(){
        $userId = Auth::id();
        $sumaTotalGastos = Gasto::where('usuario_id', $userId)->sum('monto');
        $sumaTotalIngresos = Ingreso::where('usuario_id', $userId)->sum('monto');
        return response()->json([
            'suma_total_gastos' => $sumaTotalGastos,
            'suma_total_ingresos' => $sumaTotalIngresos,
        ]);
    }
